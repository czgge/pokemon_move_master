...
async function generatePuzzles() {
  ...
  const MAX_PUZZLES_PER_POKEMON = 3; // Limit per Pokemon before prioritizing others
  
  console.log(`\n${'='.repeat(70)}`);
  console.log(`🚀 FAST PUZZLE GENERATION - Generation ${gen}`);
  console.log(`   Target: ${MAX_PUZZLES.toLocaleString()} puzzles`);
  console.log(`   Strategy: Rare moves + Pokemon distribution`);
  ...
  for (let i = 0; i < filteredPokemon.length; i++) {
    ...
    if ((i + 1) % 50 === 0 || pokemonUnique > 0) {
      console.log(`   ${pkmn.name}: ${pokemonUnique} unique puzzles`);
    }
  }
  ...
  // STEP 6: Select puzzles with Pokemon distribution priority
  console.log("✨ STEP 6: Selecting puzzles (prioritizing Pokemon diversity)...");
  ...
  // Selection algorithm: prioritize Pokemon with fewer puzzles
